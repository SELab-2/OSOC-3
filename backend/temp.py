...
import settings
from src.database.engine import _encoded_password
...
from src.database.models import (User, AuthEmail, Skill, Student,
                                 ...
                                 Project, ProjectRole, Suggestion, user_editions)
...
def fill_database(db: Session):
    ...
    # Editions
    for i in range(50):
        db.add(Edition(year=1900+i, name=f"ed{3000+i}"))
    db.commit()
    ...
    coach3: User = User(name="coach3")
    coach4: User = User(name="coach4")
    request: User = User(name="request")


    def add_randomAuth(user, email, i):
        if i % 3 == 0:
            db.add(AuthEmail(user=user, email=email, pw_hash=pw_hash))
        elif i % 3 == 1:
            db.add(AuthGitHub(user=user, email=email))
        else:
            db.add(AuthGoogle(user=user, email=email))
        db.commit()

    for i in range(5, 500):
        coachn = User(name=f"coach{i}")
        db.add(coachn)
        add_randomAuth(coachn, f"coach{i}@ngmail.com", i)
        db.execute(user_editions.insert(), [{"user_id": coachn.user_id, "edition_id": edition.edition_id}])
    db.commit()
    ...
    for i in range(750, 1250):
        ...
        db.commit()
        add_randomAuth(coachn, f"coach{i}@ngmail.com", i)
        db.add(CoachRequest(edition=edition, user=coachn))
        db.commit()
    ...
    # Coaches
    db.execute(user_editions.insert(), [
        {"user_id": coach1.user_id, "edition_id": edition.edition_id},
        {"user_id": coach2.user_id, "edition_id": edition.edition_id}
    ])
    ...
    student13: Student = Student(first_name="Melike", last_name="Süleymanoglu", preferred_name="Melike",
    ...
    student17: Student = Student(first_name="aynz", last_name="khrymy", preferred_name="Aynz",
    ...
    student30: Student = Student(first_name="Marta", last_name="Marquez", preferred_name="Marta",
                                 ...
                                 wants_to_be_student_coach=False, edition=edition, skills=[skill2, skill4, skill5])
    student31: Student = Student(first_name="Gül", last_name="Barbarosoglu", preferred_name="Gül",
    ...
    decision_email3: DecisionEmail = DecisionEmail(
        decision=EmailStatusEnum.APPLIED, student=student10, date=date.today())
    decision_email4: DecisionEmail = DecisionEmail(
        decision=EmailStatusEnum.CONTRACT_CONFIRMED, student=student11, date=date.today())
    decision_email5: DecisionEmail = DecisionEmail(
        decision=EmailStatusEnum.CONTRACT_DECLINED, student=student12, date=date.today())
    ...
    project1: Project = Project(
        name="project1", number_of_students=3, edition=edition, partners=[partner1, partner2, partner3])
    ...
    for i in range(5, 250):
        project = Project(
            name=f"project{i}", number_of_students=4, edition=edition, partners=[partner1, partner3])
        db.add(project)
    db.commit()
...
if __name__ == "__main__":
    ...
    with Session(engine) as session:
        for table in [ProjectRole, project_partners, Suggestion, project_coaches, Project, Partner, InviteLink, DecisionEmail,
                      CoachRequest, student_skills, Student, Skill, user_editions, Edition, AuthGitHub, AuthGoogle]:
            session.query(table).delete()

        session.query(AuthEmail).filter(AuthEmail.email != "seppe.maes@ugent.be").filter(AuthEmail.email != "test@mail.com").delete()
        session.commit()

        session.query(User).filter(User.name != "Seppe").filter(User.name != "test").delete()
        session.commit()
