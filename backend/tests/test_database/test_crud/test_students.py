...
async def test_get_all_students(database_with_data: AsyncSession):
    ...
    edition: Edition = (await database_with_data.execute(select(
        Edition).where(Edition.edition_id == 1))).scalars().one()
...
async def test_search_students_on_first_name(database_with_data: AsyncSession):
    ...
    edition: Edition = (await database_with_data.execute(select(
        Edition).where(Edition.edition_id == 1))).scalars().one()
    students = await get_students(database_with_data, edition,
...
async def test_search_students_one_skill(database_with_data: AsyncSession):
    ...
    edition: Edition = (await database_with_data.execute(select(
        Edition).where(Edition.edition_id == 1))).scalars().one()
    skill: Skill = (await database_with_data.execute(select(
        Skill).where(Skill.name == "skill1"))).scalars().one()
    students = await get_students(database_with_data, edition,
...
async def test_search_students_multiple_skills(database_with_data: AsyncSession):
    ...
    edition: Edition = (await database_with_data.execute(select(
        Edition).where(Edition.edition_id == 1))).scalars().one()
    skills: list[Skill] = (await database_with_data.execute(select(
        Skill).where(Skill.description == "important"))).scalars().all()
    students = await get_students(database_with_data, edition,
