...
async def register_email(user: EmailRegister, db: AsyncSession = Depends(get_session),
                         edition: Edition = Depends(get_latest_edition)):
    """
    Register a new account using the email/password format.
    """
    await create_request_email(db, user, edition)


@registration_router.post("/github", status_code=status.HTTP_201_CREATED)
async def register_github(user: GitHubRegister, db: Session = Depends(get_session),
                          http_session: ClientSession = Depends(get_http_session),
                          edition: Edition = Depends(get_latest_edition)):
    """
    Register a new account using GitHub OAuth.
    """
    access_token_data = await get_github_access_token(http_session, user.code)
