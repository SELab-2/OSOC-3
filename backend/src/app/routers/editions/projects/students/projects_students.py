...
async def change_project_role(student_id: int, skill_id: int, db: Session = Depends(get_session),
                              project: Project = Depends(get_project)):
    """
    Change the role a student is drafted for in a project.
    """
    logic_change_project_role(db, project, student_id, skill_id)


@project_students_router.post("/{student_id}", status_code=status.HTTP_201_CREATED, response_class=Response)
async def add_student_to_project(student_id: int, skill_id: int, drafter_id: int, db: Session = Depends(get_session),
                                 project: Project = Depends(get_project)):
    """
    ...
    """
    logic_add_student_project(db, project, student_id, skill_id, drafter_id)
