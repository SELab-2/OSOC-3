...
async def remove_student_from_project(student_id: int, db: AsyncSession = Depends(get_session),
                                      project: Project = Depends(get_project)):
    """
    Remove a student from a project.
    """
    await logic.remove_student_project(db, project, student_id)
...
async def change_project_role(student_id: int, input_sr: InputStudentRole, db: AsyncSession = Depends(get_session),
                              project: Project = Depends(get_project), user: User = Depends(require_coach)):
    """
    Change the role a student is drafted for in a project.
    """
    await logic.change_project_role(db, project, student_id, input_sr.skill_id, user.user_id)


@project_students_router.post("/{student_id}", status_code=status.HTTP_201_CREATED, response_class=Response,
                              dependencies=[Depends(get_latest_edition)])
async def add_student_to_project(student_id: int, input_sr: InputStudentRole, db: AsyncSession = Depends(get_session),
                                 project: Project = Depends(get_project), user: User = Depends(require_coach)):
    """
    ...
    """
    await logic.add_student_project(db, project, student_id, input_sr.skill_id, user.user_id)
...
async def confirm_project_role(student_id: int, db: AsyncSession = Depends(get_session),
                               project: Project = Depends(get_project)):
    """
    Definitively add a student to a project (confirm its role).
    This can only be performed by an admin.
    """
    await logic.confirm_project_role(db, project, student_id)
