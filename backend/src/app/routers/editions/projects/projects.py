...
from src.app.logic.projects import get_project_list, logic_create_project, logic_get_project, logic_delete_project, \
...
async def get_projects(db: Session = Depends(get_session), edition: Edition = Depends(get_edition)):
    ...
    """
    return get_project_list(db, edition)


@projects_router.post("/", status_code=status.HTTP_201_CREATED, response_class=Response)
async def create_project(name: str, number_of_students: int, db: Session = Depends(get_session), edition: Edition = Depends(get_edition)):
    """
    Create a new project.users
    """
    logic_create_project(db, edition, name, number_of_students)
...
async def get_project(project_id: int, db: Session = Depends(get_session)):
    ...
    """
    return logic_get_project(db, project_id)


@projects_router.patch("/{project_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
async def patch_project(project_id: int, name: str, number_of_students: int, db: Session = Depends(get_session)):
    """
    Update a project, changing some fields.
    """
    logic_patch_project(db, project_id, name, number_of_students)
