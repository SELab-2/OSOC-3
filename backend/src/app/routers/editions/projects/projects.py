...
async def get_projects(db: AsyncSession = Depends(get_session), edition: Edition = Depends(get_edition),
...
async def create_project(input_project: InputProject,
                         db: AsyncSession = Depends(get_session), edition: Edition = Depends(get_latest_edition)):
    """
...
async def delete_project(project_id: int, db: AsyncSession = Depends(get_session)):
    ...
    """
    return await logic.delete_project(db, project_id)
...
async def patch_project(project_id: int, input_project: InputProject, db: AsyncSession = Depends(get_session)):
    """
    Update a project, changing some fields.
    """
    await logic.patch_project(db, project_id, input_project)
