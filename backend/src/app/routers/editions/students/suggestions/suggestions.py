...
async def create_suggestion(new_suggestion: NewSuggestion, student: Student = Depends(get_student),
                            db: Session = Depends(get_session), user: User = Depends(require_authorization)):
    """
...
async def delete_suggestion(db: Session = Depends(get_session), user: User = Depends(require_authorization),
...
async def edit_suggestion(new_suggestion: NewSuggestion, db: Session = Depends(get_session),
                        user: User = Depends(require_authorization), suggestion: Suggestion = Depends(get_suggestion)):
    """
