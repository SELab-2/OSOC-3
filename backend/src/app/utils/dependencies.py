...
from src.database.crud.invites import get_invite_link_by_id
...
def get_invite_link(invite_id: int, db: Session = Depends(get_session)) -> InviteLink:
    """Get an invite link from the database, given the id in the path"""
    return get_invite_link_by_id(db, invite_id)
